import boto3
import paramiko
# This lambda function received SNS notification when spot EC2 instance terminates. 
# And on termination it rotates the logs in that ec2 by doing ssh to that box.

def ecs_spot_interruption_handler(event, context):
    ...
    s3_client.download_file('bsft-ci-packages','ansible/bsft_ansible', '/tmp/keyname.pem')
